...
from constants import MessageType, MessageField, DisconnectField
from deserialize_service import DeserializeService
from transaction import Transaction
from wallet import load_wallet, pubkey_to_address

class Node:
    def __init__(self, host: str, port: int, is_miner: bool = False, wallet_file="my_wallet.txt"):
        self._host = host
        self._port = port
        self.peers = set()
        ...
        self._external_ip = self._get_local_ip()
        self._is_miner = is_miner
    ...
    def _request_chain(self, peer) -> None:
        try:
            with socket.socket() as s:
                s.connect(peer)
